...
#constants = Constants('gpadmin', 'btq6', 'localhost', 'dwh', 'n.sangeeth@boutiqaat.com', ['n.sangeeth@boutiqaat.com'], 'email-smtp.eu-west-1.amazonaws.com', 'AKIAIHZS7GROQBF4IZYQ', 'password')
constants = Constants('gpadmin', 'btq6', '50.112.233.194', 'dwh', 'n.sangeeth@boutiqaat.com', ['n.sangeeth@boutiqaat.com'], 'smtp.sendgrid.net', 'boutiqaat', 'Boutiqaat@111')
print("seperator ======>")
SEPERATOR = '<h3> ==================================== </h3>'


def query_ofs_slave(query):
    conn = MySQLdb.connect(host='10.0.2.61', user='readonly', password='ZRxtxJCR3nLlNYgA', charset='utf8')
...
def query_master_ofs(query):
    conn = MySQLdb.connect(host='10.0.3.111', user='biteam_read', password='jksakjsk^&19', charset='utf8')
...
def query_nav_slave(query):
    engine = create_engine('mssql+pymssql://BIJOB:Btqkw1234@54.72.51.25/Boutiqaat_Live')
    conn = engine.connect()
...
def query_bi_app(query):
    #engine = create_engine('postgresql+psycopg2://gpadmin:btq6@172.21.0.137:6432/dwh')
    conn = MySQLdb.connect(host='172.31.40.80', user='master', password='sp3lVuPw', charset='utf8')
...
def mailer():
    ...
    #recipients = ['a.singh@boutiqaat.com','k.sharma@boutiqaat.com']
    recipients = ['g.rai@boutiqaat.com','a.verma@boutiqaat.com', 'm.selvaraj@boutiqaat.com', 'y.raghav@boutiqaat.com', 'h.padode@boutiqaat.com', 'n.sangeeth@boutiqaat.com','a.singh@boutiqaat.com']
    msg = MIMEMultipart('mixed')
    msg['Subject'] = "DQ Report Developed On New System"
    msg['From'] = 'BTQINSIGHTS@boutiqaat.com'
    ...
    print("here")
    server.sendmail('k.sharma@boutiqaat.com', recipients, msg.as_string())
    server.close()
